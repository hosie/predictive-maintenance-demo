...
public class MQTTConnectorFactory extends ConnectorFactory {
    ...
    private MQTTStats stats = new MQTTStats();
    ...
    public void initialize(String providerName, Properties providerProps) throws MbException {
        ...
        try {
			...
			this.useFilePersistence = "file".equalsIgnoreCase(providerProps.getProperty("property2"));
		} finally {
			writeServiceTraceExit(clsName, "initialize", "Exit");
		}
    }
    
    public MqttClientPersistence getClientPersistence() throws MbException {
		if (this.useFilePersistence) {
			...
			return new MqttDefaultFilePersistence(cpDir);
		}
     ...
     */
    @Override
    public OutputConnector createOutputConnector(String name, Properties nodeProps,SecurityIdentity securityIdentity) throws MbException {
        ...
        try {
			...
			return new MQTTOutputConnector(this, name, nodeProps,securityIdentity);
		} finally {
     ...
     */
    @Override
    public InputConnector createInputConnector(String name, Properties nodeProps,SecurityIdentity securityIdentity) throws MbException {
        ...
        try {
			...
			return new MQTTInputConnector(this, name, nodeProps,securityIdentity);
		} finally {
	...
	public int getQos(Properties props) throws MbRecoverableException {
        writeServiceTraceEntry(clsName, "getQos", "Entry");
		int qos = 0; 
		try {
			qos = Integer.parseInt(props.getProperty("qos",
				props.getProperty("qualityOfService", DEFAULT_QOS)));
		} catch (NumberFormatException e) {
			getContainerServices().throwMbRecoverableException(
				"2112",	new String[] { "not a valid integer", ""+qos });
		} 
		writeServiceTraceData(clsName, "getQos", "MQTT QOS: " + qos);
        writeServiceTraceExit(clsName, "getQos", "Exit");
		return qos;
	}
	...
	@Override
	public String[] getConnectorStatNames() {
		return stats.getStatsNames();
	}
}

...
(function(){
  ...
  (function(){
    var flowStatsWidget = function(options){
      return {
        ...
        yTicks:options.iibYTicks || 5,
        iibFlowName : options.iibFlowName,
        aspectRatio : 1/4,
        renderStaticD3:function (canvas,data){
  ...
  */
  ...
  function d3UtilFactory(iibIntegrationBus,iibSubscriber){
    ...
    function renderWidget(widget,iElement){
      ...
      if (widget.iibSimulation){
        ...
        iibSubscriber.subscribe(topic,widget,function(message){
          ...
          widget.renderDynamicD3(canvas,widget.data);          
        });
  ...
  function iibSubscriber(){
    ...
    factory.subscribe=function(topic,scope,onMessageArrived){
      ...
      clientIndex++;
      var clientId="dashboard" + (clientIndex).toString();
      console.log("client id="+clientId);

      scope.client = new Paho.MQTT.Client(host, port, clientId);
      var client = scope.client;

      client.onConnectionLost = onConnectionLost;
      client.onMessageArrived = function(message){
        try{
          onMessageArrived(message,scope);
        }catch (error){
          //TODO
      ...
      var subscription={
        ...
        unsubscribe:function(){
          console.log("unsubscribing " + this.id);
        }        
      }
      return subscription;
    };
  ...
  function iibFlowMonitoringDirective($rootScope){
    console.log("hello");
    return {
      restrict: 'AC',
      template:'<h3>{{iibFlowName}} Active Execution Groups</h3>',
        //TODO - add require:'ngEventTopic and define a directive and controller for that?
      scope:{
        iibFlowName:'@',
        iibIntegrationNodeName:'@',
        iibIntegrationServerName:'@',
        iibSimulation:'@'
      },
      link: link,
      controller: iibFlowMonitoringController
    };

    function link(scope,iElement,iAttrs){
      console.log("link flow monitoring");
      console.dir(scope);
      scope.iibEventCount=1;
      intitialiseD3(scope,iElement);
      scope.$watch('activeEgs',function(){
        draw(scope,iElement);
      },true);
      
    }

    //TODO - is there anything here that could go into the compile stage? 
    function intitialiseD3(scope,element){
      //TODO - move the following few lines into a D3 service? up to -- end of reusable section --
      var fullWidth = d3.select(element[0]).node().offsetWidth,
        fullHeight = Math.max(200,d3.select(element[0]).node().offsetHeight);

      //TODO make responsive. max margins at these numbers but respond to smaller element sizes with a margin as a fraction of element size
...
*/
      var servers= scope.g.selectAll(".activeServer")
                  .data(scope.activeEgs);
      ...
      var serversGroup = servers.enter().append("g").attr("class","activeServer");

      serversGroup.append("rect")
         .attr("x",0)
         .attr("y",function(d,i){
           return i*40;
         })
      ...
      console.dir(scope.activeEgs);

      scope.g.selectAll(".activeServer rect")
               .attr("fill","#FF0000");
    }    

    function iibFlowMonitoringController($scope,iibSubscriber){
      console.log("iibFlowMonitoringController");

      $scope.activeEgs=[];
      //must be called from within an 'apply'
      $scope.activeEgs.touch=function(egName){
        var currentEg=undefined;
        this.forEach(function(item){
          if(item.name===egName) {
            currentEg=item;
          }
        });
        if(currentEg==undefined) {
          currentEg = {
            name:egName
          };          
          this.push(currentEg);
        ...
        //touch mark the time for the existing or new EG
        var now=new Date();
        currentEg.time =  now;
        
        var self=this;
        //Set it to expire if it is not touched in the meantime
        setTimeout(function(){
          if(currentEg.time==now){
            console.log("expriring " + currentEg.name);
            var thisIndex;
            ...
            //TODO handle thisIndex undefined
            $scope.$apply(function(){          
              self.splice(thisIndex,1);
            });
      ...
      var topic = getTopic({
                             flowName:$scope.iibFlowName,
                             integrationServerName:$scope.iibIntegrationServerName,
                             integrationNodeName:$scope.iibIntegrationNodeName
                           });
      ...
      function getTopic(options){
        ...
        var integrationServerName = options.integrationServerName || '+';
        var integrationNodeName   = options.integrationNodeName || '+';
        console.log("flowName = " + flowName);
        //TODO - cannot wildcard the broker name when using the broker's websockets MQTT
        return 'IBM/IntegrationBus/' + integrationNodeName + '/Monitoring/' + integrationServerName + '/' + flowName;
      }
      
      function onMessageArrived(message,scope) {
      
        var payloadString = message.payloadString;

        console.log("onMessageArrived");
        //TODO parse XML
        var payloadObject = (new window.DOMParser() ).parseFromString(payloadString, "text/xml");
        //console.dir(payloadObject);
        var jqpayloadObject = jQuery.parseXML( payloadString )
        //console.dir(jqpayloadObject);
        var egName = jQuery(jqpayloadObject).find("executionGroup").attr("wmb:name");
        console.dir(egName);
        
        scope.$apply(function(){
          scope.activeEgs.touch(egName)
    ...
    function iibFlowMonitoringSimulationController($scope){
      console.log("iibFlowMonitoringSimulationController");
      //set eg1 to be active regularly
      $scope.activeEgs.touch("eg1");
      setInterval(function(){
        //console.log("scope - ");
        //console.dir($scope);
        console.log("touching eg1");
        $scope.activeEgs.touch("eg1");
      },10000);

      //mock
      //set eg2 to go active, then become inactive
      setTimeout(function(){

        console.log("touching eg1");
        $scope.activeEgs.touch("eg2");
      },10000);

    }
  }

  function iibWidgetDirective($rootScope,iibSubscriber,d3Util){
